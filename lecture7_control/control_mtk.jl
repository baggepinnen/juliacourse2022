...
# ╔═╡ 00026d4f-f7e7-4e78-99a8-192443f3fc8b
begin
	using ShortCodes
	ShortCodes.YouTube("favQKOyyx4o")
end

# ╔═╡ 95a1a0a2-5901-11ed-166a-8d7b4654839d
begin
	using ModelingToolkit, ControlSystemsBase, PlutoUI, Plots, OrdinaryDiffEq, LinearAlgebra, Kroki
...
# ╔═╡ b77e4fdb-007c-48b6-a2eb-17bc99653cb9
using ControlSystemsMTK

# ╔═╡ dbc4a26d-60b0-4fc8-81b1-b66063b30eb2
md"""
# ModelingToolkit and JuliaControl

In this example, we will build a model of a DCPM electrical motor and control it using a PI controller. We will simulate the system and compute the sensitivity function to verify robustness.

For a more elaborate example, see the video below.
"""

# ╔═╡ bc7b3557-d060-45bc-a32c-f12b7e5131e4
...
# ╔═╡ b3f853f6-dfe2-4af2-974c-0ca28191d496
@variables t
...
```
                            ...
                            --+->|  C  +---->|  P  +-+->
                             -|  |     |     |     | |
                              ...
                              +----------------------+
```
where ``P(s)`` is a DCPM electrical motor and ``C(s)`` is a PI controller. The schematics for the motor looks like this
![motor](https://user-images.githubusercontent.com/50108075/196108356-0e8605e3-61a9-4006-8559-786252e55928.png)
"""

# ╔═╡ 3b78f45a-289d-4b4c-a05f-06a8e420c1b6
function Motor(; name, R = 0.5, L = 4.5e-3, k = 0.5, J = 0.02, f = 0.01)
    @named ground = Ground()
    @named source = Voltage()
    @named R1 = Resistor(R = R) # R = 0.5 # [Ohm] armature resistance
    @named L1 = Inductor(L = L) # L = 4.5e-3 # [H] armature inductance
    @named emf = EMF(k = k)     # k = 0.5 # [N.m/A] motor constant
    @named fixed = Fixed()
    @named load = Torque(use_support = false)
    @named inertia = Inertia(J = J) # J = 0.02 # [kg.m²] inertia
    @named friction = Damper(d = f) # f = 0.01 # [N.m.s/rad] friction coefficient
...
md"""
## Add a PI controller
We will use the block [`ModelingToolkitStandardLibrary.Blocks.LimPI`](https://docs.sciml.ai/ModelingToolkitStandardLibrary/stable/API/blocks/#ModelingToolkitStandardLibrary.Blocks.LimPI)
"""
...
connections = [
            ...
            connect(speed_sensor.w, :y, feedback.input2) # Add analysis point :y
            connect(load_step.output, motor.load.tau)
            connect(feedback.output, pi_controller.err_input)
            connect(pi_controller.ctr_output, :u, motor.source.V)]; # Add analysis point :u
...
begin
	...
	p1 = plot(sol.t, sol[motor.inertia.w], ylabel = "Angular Vel. in rad/s",
	                label = "Measurement", title = "DC Motor with Speed Controller", legend=:bottomright)
	plot!(sol.t, sol[ref.output.u], label = "Reference")
	plot!(sol.t, sol[motor.load.tau.u], lab = "Disturbance in Nm")
end
...
ModelingToolkit supports inserting *analysis points* into models to facilitate *linear analysis*. An analysis point can be viewed as a named connection of particular interest for causal linear analysis. An example use case for analysis points is to derive [*sensitivity functions*](https://en.wikipedia.org/wiki/Sensitivity_(control_systems)). An analysis point is created automatically if a connection is named using the syntax `connect(output, :name, input)`. Please note: the analysis point connections are inherently *causal*, the order of the connected signals in an analysis-point connection is thus important.  The documentation for analysis points is [available here](https://docs.sciml.ai/ModelingToolkitStandardLibrary/stable/API/linear_analysis/#Linear-Analysis).
...
begin
	matrices_S, simplified_sys = get_sensitivity(model, :y);
	S = ss(matrices_S...) |> minreal;
end

# ╔═╡ 09817d80-9d72-4e79-98b4-7757571118b6
Ms, ωMs = hinfnorm(S) # Peak of the sensitivity function

# ╔═╡ 6a3bf8f6-b6a6-433b-9d57-a5a4103e11ab
begin
	Lt = -ss(get_looptransfer(model, :u)[1]...)
...
md"""
Linearize the plant model only using function `linearize`:
"""

# ╔═╡ e08b87c8-36c7-4f82-a98c-2c1ddeaf8ac8
begin
	motor_c = complete(motor) # Required to be able to access variables within the system on the line below
	matrices_P, simplified_sys_P = linearize(motor_c, [motor_c.source.V.u, motor_c.load.flange.tau], [motor_c.load.flange.phi])
end

# ╔═╡ d4509b6b-7ca5-44e2-9d8f-dc985063d82a
md"Retain the signal names by linearizing using the function `named_ss`:"

# ╔═╡ 65e79a5e-ddc0-4e2a-8a73-e1ec4ec636fc
named_lsys = named_ss(motor_c, [motor_c.source.V.u, motor_c.load.flange.tau], [motor_c.load.flange.phi])

# ╔═╡ c37a2102-ebf7-49fe-9cb4-764f3122a536
html"""<style>
main {
    max-width: 60%;
}
...
ControlSystemsMTK = "~0.1.12"
Kroki = "~0.2.0"
ModelingToolkit = "~8.52.0"
ModelingToolkitStandardLibrary = "~1.12.0"
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "3bbef94eef394433cd02aab5f6a2aa61d69709e0"
...
git-tree-sha1 = "f7fbfb6100f6771413e1182012411b81c9b1149d"
...
git-tree-sha1 = "afede45ef20a32a2fd4a995a30f96a2f5cb33a51"
...
deps = ["IfElse", "LinearAlgebra", "ModelingToolkit", "Symbolics"]
git-tree-sha1 = "51cceae7a6489c069ff0276011399cd5eed3a18a"
...
git-tree-sha1 = "5677bd7aa0f499eef6bb59ddc2f68fb4bbd533b3"
